# v2 of the fork: canonical-key dedupe, nearest-mass fill to 25, strong library hits ranked first.
...
class CFG:
    ...
    LIB_HARD     = 0.80   # v2: library entropy similarity at or above this goes to the top
...
for gi, (mid, sub) in enumerate(mols):
    ...
    if len(nms):
        ...
        if len(cand):
            ...
            p = rank_proba(X)
            # v2: a strong direct library match outranks the learned ranker
            strong = lv >= CFG.LIB_HARD
            score = p + np.where(strong, 10.0 + lv, 0.0)
