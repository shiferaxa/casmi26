...
- Kaggle auth on this box is the token file in the user's .kaggle folder (KGAT token, username amhashiferaw). Rotate it after the competition, it was pasted in chat once.
- No CUDA on this machine (AMD integrated GPU). Train models on Kaggle GPU, not here.
