...
- Fingerprint checkpoints for the harness come only from the folder given by `--models` (default `data/public/casmi26-fp-models-v2`). Never rely on a sweep of `data/`: every downloaded or smoke-tested `fp_*.pt` under it would be ensembled silently (this corrupted one evaluation on Sep 21). Public v2 and v4 share file names, so a combined folder needs renamed copies.
...
- `.github/workflows/kaggle-train.yml` pushes a kernel folder to Kaggle. Runs every Saturday 00:05 UTC (the GPU quota refresh) for `kaggle/train_fp_v2`, and on demand from the Actions tab with any kernel folder and accelerator. Pushing a kernel that is already running restarts it as a new version.
...
Sep 21 2026: public LB 0.339, rank 60 of 1,113 (board moved: rank 10 is 0.361). Kernel fork-v7 (CPU, no prune, analog power 3, public v2 checkpoints) is the pending submission, backed by np-examples class 2 0.786 versus 0.758. Own fingerprint model: v1 recipe reached 0.317 alone (public 0.473); v2 recipe (`kaggle/train_fp_v2`, hard negatives, merge views, augmentation) is smoke-tested and gets pushed automatically Saturday 00:05 UTC by the kaggle-train workflow; the checkpoint workflow publishes its weights to the dataset casmi26-fp-own. Next after v2 trains: evaluate it alone with `scripts/eval_channel4.py --models`, and only ensemble if it beats the public model alone. See the log at the bottom of `docs/plan.md`.
