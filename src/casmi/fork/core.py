"""Verbatim transplant of the public 0.339 pipeline (haideptry / prvsiyan, Apache 2.0 on Kaggle),
with the Kaggle-only module-level loads turned into functions so it runs locally under harness.py.
Numerics are unchanged from kaggle/fork_v2/notebook.py apart from the hide_key masks and the fragment cache.
Regenerated by hand from that notebook; if the notebook changes, re-run the transplant, do not edit both.
"""
...
SEARCH_ROOTS = ['/kaggle/input']  # set by harness.init() for local runs
...
def find_file(name):
    ...
    if not hits:
        raise FileNotFoundError(f"File not found under {SEARCH_ROOTS}: {name}")
...
# ===================================================================================
BITS = None  # loaded by harness.init()
_g = {}
...
def build_candidate_pool(TRAIN, workers=4):
    t0 = time.time()
...
def lib_sim(L, specs, target, hide_key=None):
    ...
    if hide_key is not None and len(cand):
        cand = cand[L['ik'][cand] != hide_key]
...
def analog_sim(L, specs, target, rep, rep_key, rep_nm, hide_key=None):
    ...
    if hide_key is not None and len(cand):
        keep = ckey != hide_key
        cand, shift, ckey = cand[keep], shift[keep], ckey[keep]
    if len(cand) == 0: return []
    agg = {}
    for mz, it in specs:
        qm, qp = clean_spectrum(mz, it)
...
FRAG_CACHE = {}  # smiles -> fragment mass array (pure function of the structure)
FRAG_POOL = None  # persistent multiprocessing pool set by harness.init()

def frag_scores(cand_smiles, specs, mode, workers=4):
    if not HAVE_RDKIT:
        return np.zeros(len(cand_smiles), np.float32)
    todo = [s for s in dict.fromkeys(cand_smiles) if s not in FRAG_CACHE]
    if todo:
        if FRAG_POOL is not None:
            res = FRAG_POOL.map(_frag_masses_wrapper, todo, chunksize=8)
        else:
            with MPool(workers) as mp:
                res = mp.map(_frag_masses_wrapper, todo, chunksize=8)
        FRAG_CACHE.update(zip(todo, res))
    frags = [FRAG_CACHE[s] for s in cand_smiles]
    peaks = []
    for mz, it in specs:
...
def load_neural_models():
    global _MODEL
    paths = sorted({h for r in SEARCH_ROOTS for h in glob.glob(os.path.join(r, '**', 'fp_*.pt'), recursive=True)})
    if not paths:
...
# Ranker ensemble, fitted by harness.init()
RANKERS = []
NFEAT = 31

def fit_rankers(rank_train_path):
    global RANKERS, NFEAT
    ...
    for w1 in CFG.W1_PRIORS:
        ...
        for sd in CFG.SEEDS:
            ...
            RANKERS.append(m)
    print(f"[INFO] ranker ensemble: {len(RANKERS)} GBMs on {z['X'].shape[0]:,} rows x {NFEAT} features", flush=True)
